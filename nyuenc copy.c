    ...
    if (jobs > 1) {
        ...
        void *run_thread(void *args) {
            ...
            while (1) {
                ...
                pthread_cond_signal(&output_queue->not_empty);
        ...
        for (int i = 0; i < chunk_count; i++) {
            ...
            while (OQ->head == NULL) {
                printf("Waiting for output %d\n", i);
                pthread_cond_wait(&OQ->not_empty, &OQ->mutex);
            }
            printf("Output Queue head: %d\n", OQ->head->chunk_index);
            ...
            //         prev->size -= 2;
            //         curr->addr[1] = prev_last_count + curr_first_count;
            //     }
            //     write(STDOUT_FILENO, prev->addr, prev->size);
            // }
            // write the output to stdout
            // printf("Received output %d-%d\n", output->file_index, output->section_index);
            // fflush(stdout);
            // write(STDOUT_FILENO, output->addr, output->size);
            // printf("\n");
            // fflush(stdout);
            ...
            // add_output_to_list(output_list, output);
            // output_array[output->file_index][output->section_index] = output;
            // prev = curr;
        }
